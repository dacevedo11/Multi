...
{
	// if the clip is depleted, return
	if (CurrentBullets <= 0)
	{
		return;
	}
...
{
	SetCurrentBullets(CurrentBullets + Amount);
...
bool AShooterWeapon::bIsFullAmmo() const
{
	return CurrentBullets >= MagazineSize;
}

void AShooterWeapon::SetCurrentBullets(int32 InCurrentBullets)
{
	CurrentBullets = InCurrentBullets;
